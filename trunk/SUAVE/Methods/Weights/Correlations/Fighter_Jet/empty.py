...
def empty(vehicle):
    ...
    if not vehicle.wings.has_key('main_wing'):
        ...
        if vehicle.wings['main_wing'].tag == "delta":
            K_dw = 0.768
        else:  
            K_dw = 1.0        
        if len(vehicle.wings['main_wing'].Segments.keys())>0:
            K_vs = 1.19
        else: 
            K_vs = 1.0
        wt_wing    = wing_main(S_gross_w,b,lambda_w,t_c_w,sweep_w,Nult,TOW,wt_zf,K_dw,K_vs)
    ...
    if not vehicle.wings.has_key('horizontal_stabilizer'):
        ...
        wt_tail_horizontal = tail_horizontal(b_h,w_fus,Nult,TOW,wt_zf,S_h)         
    ...
    if not vehicle.wings.has_key('vertical_stabilizer'):   
        ...
        output_3   = tail_vertical(S_v,Nult,b_v,TOW,t_c_v,sweep_v,S_gross_w,t_tail,rudder_fraction = 0.25) 
    ...
    wt_fuselage        = fuselage(w_fus,h_fus,l_fus,Nlim,TOW,wt_zf,K_dwf) 
    output_2           = systems(num_seats, ctrl_type, S_h, S_v, S_gross_w)  
